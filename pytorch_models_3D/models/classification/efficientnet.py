...
class MBConvBlock3D(nn.Module):
    ...
    def __init__(self, block_args, global_params):
        ...
        # Expansion phase
        inp = self._block_args.input_filters  # number of x channels
...
class EfficientNet3D(nn.Module):
    ...
    """
    def __init__(self, features_only: bool, blocks_args=None, global_params=None, in_channels=3, num_classes=1000):
        super().__init__()
        ...
        # Build blocks
        self._blocks = nn.ModuleList(list())
        for block_args in self._blocks_args:

            # Update block x and output filters based on depth multiplier.
        ...
        # Head
        in_channels = self._blocks_args[-1].output_filters  # output of final block
        out_channels = round_filters(num_classes, self._global_params)
...
class Conv3dDynamicSamePadding(nn.Conv3d):
    ...
    def forward(self, x):
        ...
        if pad_h > 0 or pad_w > 0 or pad_z > 0:
            x = F.pad(
                x, [pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2, pad_z // 2, pad_z - pad_z // 2]
            )
...
def efficientnet_b0(num_classes: int, in_channels: int):
    return EfficientNet3D.from_name(
        model_name="efficientnet-b0", override_params={'num_classes': num_classes}, in_channels=in_channels
    )


def efficientnet_b1(num_classes: int, in_channels: int):
    return EfficientNet3D.from_name(
        model_name="efficientnet-b1", override_params={'num_classes': num_classes}, in_channels=in_channels
    )


def efficientnet_b2(num_classes: int, in_channels: int):
    return EfficientNet3D.from_name(
        model_name="efficientnet-b2", override_params={'num_classes': num_classes}, in_channels=in_channels
    )


def efficientnet_b3(num_classes: int, in_channels: int):
    return EfficientNet3D.from_name(
        model_name="efficientnet-b3", override_params={'num_classes': num_classes}, in_channels=in_channels
    )


def efficientnet_b4(num_classes: int, in_channels: int):
    return EfficientNet3D.from_name(
        model_name="efficientnet-b4", override_params={'num_classes': num_classes}, in_channels=in_channels
    )


def efficientnet_b5(num_classes: int, in_channels: int):
    return EfficientNet3D.from_name(
        model_name="efficientnet-b5", override_params={'num_classes': num_classes}, in_channels=in_channels
    )


def efficientnet_b6(num_classes: int, in_channels: int):
    return EfficientNet3D.from_name(
        model_name="efficientnet-b6", override_params={'num_classes': num_classes}, in_channels=in_channels
    )


def efficientnet_b7(num_classes: int, in_channels: int):
    return EfficientNet3D.from_name(
        model_name="efficientnet-b7", override_params={'num_classes': num_classes}, in_channels=in_channels
    )


def efficientnet_b8(num_classes: int, in_channels: int):
    return EfficientNet3D.from_name(
        model_name="efficientnet-b8", override_params={'num_classes': num_classes}, in_channels=in_channels
    )


def efficientnet_l2(num_classes: int, in_channels: int):
    return EfficientNet3D.from_name(
        model_name="efficientnet-l2", override_params={'num_classes': num_classes}, in_channels=in_channels
    )
